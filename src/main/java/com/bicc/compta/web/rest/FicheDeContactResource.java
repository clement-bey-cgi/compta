...
public class FicheDeContactResource {
    ...
    public ResponseEntity<List<FicheDeContact>> getAllFicheDeContacts(Pageable pageable, @RequestParam(required = false) String filter) {
        if ("cv-is-null".equals(filter)) {
            log.debug("REST request to get all FicheDeContacts where cv is null");
            return new ResponseEntity<>(ficheDeContactService.findAllWhereCvIsNull(),
                    HttpStatus.OK);
        }
        log.debug("REST request to get a page of FicheDeContacts");
        Page<FicheDeContact> page = ficheDeContactService.findAll(pageable);
